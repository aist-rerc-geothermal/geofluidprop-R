...
UTEST_WEAK UTEST_OVERLOADABLE void utest_type_printer(float f) {
  UTEST_PRINTF("%.12e", UTEST_CAST(double, f));
}
...
#define EXPECT_NEAR0(x_, y_, eps)                                                \
  {                                                                              \
    double __x = (x_), __y = (y_);                                                       \
    if ((__x==0.0 && fabs(__x-__y)>eps) || (__x!=0.0 && fabs(__x-__y)>fabs(__x)*eps)) {      \
      UTEST_PRINTF("%s:%u: Failure\n", __FILE__, __LINE__);                      \
      UTEST_PRINTF("  Expected : %.12e\n", __x);                                   \
      UTEST_PRINTF("    Actual : %.12e\n", __y);                                   \
      UTEST_PRINTF("     Error : %.12e (%.2e%%)\n", __y-__x, __x==0?0:1e2*fabs((__y-__x)/__x));                                 \
      *utest_result = 1;                                                         \
    }                                                                            \
  }

#define EXPECT_NEAR(x, y) EXPECT_NEAR0(x, y, 1.e-8)
#define EXPECT_NEAR3(x, y) EXPECT_NEAR0(x, y, 1.e-3)
